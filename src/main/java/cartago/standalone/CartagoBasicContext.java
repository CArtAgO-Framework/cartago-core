package cartago.standalone;

import java.net.URI;

import cartago.*;
import cartago.events.*;
import cartago.utils.agent.ActionFailedException;
import cartago.utils.agent.ActionFeedback;
import cartago.utils.agent.ActionFeedbackQueue;
import cartago.utils.agent.ArtifactObsProperty;
import cartago.utils.agent.ObsEventQueue;
import cartago.utils.agent.ObsPropMap;
import cartago.utils.agent.Percept;
...
public class CartagoBasicContext {
	...
	public CartagoBasicContext(String name){
		...
		try {
			session = CartagoEnvironmentStandalone.getInstance().startSession(new cartago.AgentIdCredential(name), agentCallback);
			this.implicitWspId = session.getJoinedWorkspaces().get(0);
		} catch (Exception ex){
			ex.printStackTrace();
		}
	}
	 ...
	 *//*
	public CartagoBasicContext(String name, URI wspURI){
		super();
		this.name = name;
		agentCallback = new CartagoListener();
		actionFeedbackQueue = new ActionFeedbackQueue();
		...
		obsPropMap = new ObsPropMap();
		try {
			session = CartagoEnvironment.getInstance().joinWorkspace(wspURI, new cartago.AgentIdCredential(name), agentCallback);
			this.implicitWspId = session.getJoinedWorkspaces().get(0);
		} catch (Exception ex){
			ex.printStackTrace();
		}
