...
public class Workspace {
	...
	/**
	 * 
	 * Getting a raw artifact reference
	 * 
	 * @param artifactName
	 * @return
	 */
	public Artifact getRawArtifactReference(String artifactName) {
		synchronized (artifactMap){
			ArtifactDescriptor des = artifactMap.get(artifactName);
			if (des != null) {
				return des.getArtifact();
			} else {
				return null;
			}
		}
	...
	/***********************************************************************************************
	 * 
	 * Methods for Joining, Quitting this workspace
	 * 
	 ***********************************************************************************************/
	...
	public ICartagoContext joinWorkspace(AgentCredential cred, String artBodyClassName, ICartagoCallback eventListener) throws CartagoException {
		...
		synchronized (joinedAgents){
			...
			if (joinOK){
				context = new AgentBody(userId, cred, this, eventListener); 
	...
	/**
	 * 
	 * Quit an agent from this workspace.
	 * 
	 * @param userId
	 * @throws CartagoException
	 */
	public void quitWorkspace(AgentId userId) throws CartagoException {
		boolean quitOk = true;
		String failureMsg = "no msg";
	...
	HashMap<String,AgentBody> getJoinedAgents(){
		return this.joinedAgents;
	}

	public void removeGarbageBody(AgentBody ctx){
		try {
			quitWorkspace(ctx.getAgentId());
		} catch (Exception ex){
			ex.printStackTrace();
	...
	/***********************************************************************************************
	 * 
	 * Methods for Managing Artifact Factories 
	 * 
	 ***********************************************************************************************/
	...
	/***********************************************************************************************
	 * 
	 * Methods for Managing Artifacts
	 * 
	 ***********************************************************************************************/
	 ...
	 */
	public ArtifactId makeArtifact(AgentId creatorId, String name, String template, ArtifactConfig config) throws ArtifactAlreadyPresentException, UnknownArtifactTemplateException, ArtifactConfigurationFailedException {
		ArtifactId id = null;
		...
		try {
			...
			id = new ArtifactId(name,  template, this.getId(), creatorId);
			//log("NEW ID CREATED: "+id+" for "+name);

			artifact.bind(id,creatorId,this);	
			...
			try {
				...
				if (logManager.isLogging()){
					logManager.artifactCreated(System.currentTimeMillis(),id,creatorId);
				}	
	...
	public ArtifactId addArtifactToDefaultSet(String name, String template, ArtifactConfig config) throws ArtifactAlreadyPresentException, UnknownArtifactTemplateException, ArtifactConfigurationFailedException {
		return this.makeArtifact(wspManager.getAgentId(), name, template, config );
	}
	 ...
	 * @param test
	 */
	public ArtifactId lookupLocalArtifact(String name) throws UnknownArtifactException, ArtifactNotAvailableException{
		synchronized (artifactMap){
	...
	public ArtifactId lookupArtifact(AgentId userId, String name) throws UnknownArtifactException, ArtifactNotAvailableException, WorkspaceNotFoundException {
		return this.lookupLocalArtifact(name);
	}

	public ArtifactId lookupArtifact(String name) throws UnknownArtifactException, ArtifactNotAvailableException, WorkspaceNotFoundException {
		return this.lookupLocalArtifact(name);
	}
	...
	*/
	...
	/**
	 * 
	 * Get the identifier of the Workspace artifact.
	 * 
	 * @return
	 */
	...
	public void notifyJoinWSPRequestCompleted(ICartagoCallback listener, long actionId, ArtifactId aid, Op op, WorkspaceId wspId, String sid) {
		ActionSucceededEvent ev = eventRegistry.makeJoinWSPRequestSucceededEvent(actionId, aid, op, wspId, sid);
		listener.notifyCartagoEvent(ev);
	}
	...
	/***********************************************************************************************
	 * 
	 * Methods for Security Manager 
	 * 
	 ***********************************************************************************************/

	/**
	 * 
	 * Enable a security manager.
	 * 
	 */
	...
	/***********************************************************************************************
	 * 
	 * Methods for AgentBody Artifact Management
	 * 
	 ***********************************************************************************************/
	...
	public AgentBody getAgentBody(AgentId agentId){
		synchronized (joinedAgents){
			AgentBody body = joinedAgents.get(agentId.getGlobalId());
			return body;
		}
	...
	/***********************************************************************************************
	 * 
	 * Workspace Rule Engine Management
	 * 
	 ***********************************************************************************************/
	...
	protected void log(String st){
		System.out.println("[Workspace " + this.getId() + "] "+st);
	}
	...
	/***********************************************************************************************
	 * 
	 * Internals about Artifact Operation Execution and Controllers
	 * 
	 ***********************************************************************************************/
